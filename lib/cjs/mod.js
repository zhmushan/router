...
class Node {
  constructor(node) {
    ...
    _insert.set(this, (path, func) => {
      let n = this;
      let c = n.children.get(path[0]);
      if (c) {
        n = __classPrivateFieldGet(c, _merge).call(c, path, func);
      } else {
        c = new Node({ path, func });
        n.children.set(path[0], c);
        n = c;
  ...
  add(path, func) {
    ...
    if (j === path.length) {
      __classPrivateFieldGet(n, _merge).call(n, "", func);
    } else {
      __classPrivateFieldGet(n, _insert).call(n, path.slice(j), func);
    }
  }
  find(path) {
    let func;
